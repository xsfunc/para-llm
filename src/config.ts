...
const baseConfigSchema = v.object({
  ...
  useChatWhitelist: v.optional(v.pipe(v.string(), v.transform(JSON.parse), v.boolean()), 'false'),
  ...
  geminiApiKey: v.string('Invalid Gemini API KEY'),
  ...
  allowedChats: v.optional(v.pipe(v.string(), v.transform(JSON.parse), v.array(v.number())), '[]'),
  botNames: v.optional(v.pipe(v.string(), v.transform(JSON.parse), v.array(v.string())), '[]'),
  replacementUsername: v.optional(v.string(), ''),
